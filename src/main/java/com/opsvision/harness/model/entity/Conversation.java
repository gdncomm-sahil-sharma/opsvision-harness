...
public class Conversation {
    ...
    /** Tokens charged on the LLM input side for this turn. Null when the
     *  upstream model didn't return usage (rare). Sum of system prompt +
     *  memory replay + tool descriptions + user message + tool results. */
    @Column(name = "prompt_tokens")
    private Integer promptTokens;

    /** Tokens generated by the LLM for this turn (final answer + any
     *  intermediate tool-call planning text). Null on missing metadata. */
    @Column(name = "completion_tokens")
    private Integer completionTokens;

    /** Sum of {@link #promptTokens} and {@link #completionTokens}. Stored
     *  separately so SQL aggregations don't have to coalesce nulls. */
    @Column(name = "total_tokens")
    private Integer totalTokens;

    /** Computed harness-side from a static per-model rate table (USD per
     *  1k tokens). Stored as NUMERIC(12,6) — six decimals fits sub-cent
     *  precision for cheap models without overflow on long chats. */
    @Column(name = "cost_usd")
    private java.math.BigDecimal costUsd;

    /** OpenAI model id reported in the response metadata (e.g.
     *  "gpt-5.4-mini-2026-04-12"). Captured per-turn because the
     *  configured model can drift over a long chat (config change,
     *  fallback, A/B). */
